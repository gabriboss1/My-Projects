...
def get_stock_price(symbol):
    try:
        ...
        return latest_price, opening_price
...
def get_stock_change(symbol):
    try:
        ...
        return latest_price, direction, f"{percentage_change:.2f}%"
    except Exception as e: 
        print(f"Error fetching stock change: {e}")
...
@app.route("/get_notable")
def get_notable():
    notable_symbols = ["AAPL", "MSFT", "GOOGL", "AMZN", "NVDA", "META", "JPM", "BRK-B","QQQ", "SPY"]
    notable_stocks = []
    for symbol in notable_symbols:
        price, direction, percentage = get_stock_change(symbol)
